...
const config: Option<AppConfig> = getConfig()

// Function parameter with optional value
function processName(name: Option<string>): string {
  return name?.toUpperCase() || "UNKNOWN"
}

// Complex object with null
const currentUser: Option<{ name: string; age: number }> = getCurrentUser()
...
class UserService {
  ...
  validateAndProcessUsers(): Option<ProcessedUser[]> {
    try {
...
interface User {
  ...
  name: string
  email: string
  age: Option<number> // This should also be Option<number>
}
...
declare function getConfig(): Option<AppConfig>
declare function getCurrentUser(): Option<{ name: string; age: number }>
